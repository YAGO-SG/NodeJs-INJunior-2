...
import { userComents } from "./user-coments.controller.js";
...
export async function comentRoutes(app: FastifyInstance) {
    ...
    app.delete('/delete/:publicId', { onRequest: [verifyJwt]}, deleteComent)
    app.patch('/update/:publicId', { onRequest: [verifyJwt]}, UpdateComent)
    app.get('/user/:publicId', userComents)
}